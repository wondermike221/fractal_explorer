...
impl Mandelbrot {
    pub fn generate_fractal(self, corners: Corners) -> Vec<u32> {
        ...
        buffer.par_iter_mut().enumerate().for_each(|(i, b)| {
            let real = interpolate(
                (i % crate::DEFAULT_WIDTH) as f64,
                0.,
                crate::DEFAULT_WIDTH as f64,
                x_min,
                x_max,
            );
            let imag = interpolate(
                (i / crate::DEFAULT_HEIGHT) as f64,
                0.,
                crate::DEFAULT_HEIGHT as f64,
                y_min,
                y_max,
            );
